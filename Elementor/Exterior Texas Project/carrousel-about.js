...
const galleryContainer = document.querySelector('.aboutgallery-container');
const galleryControlsContainer = document.querySelector('.gallery-controls');
const galleryControls = ['previous','next'];
const galleryItems = document.querySelectorAll('.aboutgallery-item');
...
class Carousel {
    ...
    updateGallery (){
        this.carouselArray.forEach(im => {
            im.classList.remove('aboutgallery-item-1');
            im.classList.remove('aboutgallery-item-2');
            im.classList.remove('aboutgallery-item-3');
            // im.classList.remove('gallery-item-4');
            // im.classList.remove('gallery-item-5');
        });

        this.carouselArray.slice(0,5).forEach((im, i) => {
            im.classList.add(`aboutgallery-item-${i+1}`);
        });
    }
